...
- Платежи — **ручной сценарий** (перевод на карту + подтверждение тренером), БЕЗ YooKassa/вебхуков/чеков ФНС.
  Один тариф «Сопровождение» (константы в `services/payments.py`). Клиент: «💳 Оплата» → реквизиты →
  «Я оплатил» → заявка тренеру; тренер подтверждает → `subscriptions` продлевается (пишется `payments`).
