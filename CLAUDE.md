...
- Платежи — **ручной сценарий** (перевод на карту + подтверждение тренером), БЕЗ YooKassa/вебхуков/чеков ФНС.
  Три тарифа (`PLANS` в `services/payments.py`): Питание+Тренировки 15000 / Тренировки 10000 / Питание 7000.
  Клиент: «💳 Оплата» → выбор тарифа → реквизиты → «Я оплатил» → заявка тренеру (с тарифом);
  тренер подтверждает → `subscriptions` продлевается на выбранный тариф (пишется `payments`).
  Тикер раз в день (12:00 локально) напоминает за ≤3 дня и ставит `past_due`. Доступ оплатой НЕ гейтим.
  Реквизиты — `PAYMENT_CARD`/`PAYMENT_RECIPIENT` в `.env` (в коде тестовые дефолты).

## Дальше (опционально)
- `program_items` — структурные пункты программы (день/порядок/упражнение/подходы/видео) вместо текста.
- Scoped-команды тренера (`BotCommandScopeChat`), чтобы клиент не видел тренерских команд в меню.
- Проверить на сервере расписание: срабатывание напоминаний и уведомлений об истечении подписки.
